...
function buildUnwatchCommand(
  pattern: string,
  groupName: string,
  port: string,
): string {
  const groupFlag =
    groupName && groupName !== "default" ? ` -t ${shellQuote(groupName)}` : "";
  const portFlag = port && port !== DEFAULT_PORT ? ` -p ${port}` : "";
  return `yome --unwatch ${shellQuote(pattern)}${groupFlag}${portFlag}`;
}
  ...
  return (
    <div className="flex flex-col items-start max-w-2xl mx-auto mt-12 p-6 text-gh-text-secondary text-sm gap-4">
      ...
      <p>
        ...
        {multiple ? "s" : ""}. Run the command{multiple ? "s" : ""} below to
        unwatch {multiple ? "them" : "it"}. The group will disappear once all
      ...
      <div className="flex flex-col gap-2 w-full">
        {patterns.map((p) => (
          <CommandRow
            key={p}
            command={buildUnwatchCommand(p, groupName, port)}
          />
        ))}
      </div>
    </div>
