...
async function addFile(path, group = "default") {
  const res = await fetch(
    `${BASE}/_/api/groups/${encodeURIComponent(group)}/files`,
    {
      method: "POST",
...
async function main() {
  ...
  const server = spawn(resolve(ROOT, "yome"), [firstFile, "-p", String(PORT)], {
    cwd: ROOT,
    stdio: "ignore",
  });
  ...
  try {
    ...
    try {
      ...
      await page1.addInitScript(() => {
        localStorage.setItem("yome-theme", "dark");
      });
    ...
    server.kill();
    // Kill any remaining yome process on the port
    try {
